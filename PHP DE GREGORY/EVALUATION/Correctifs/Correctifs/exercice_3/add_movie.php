...
if(!empty($_POST)){ // Si la super-globale $_POST n'est pas vide, le formulaire a été soumis
	...
	elseif(!is_numeric($_POST['year_of_prod']) || !strtotime($_POST['year_of_prod'])){
	...
	if(!filter_var($_POST['video'], FILTER_VALIDATE_URL, FILTER_FLAG_SCHEME_REQUIRED | FILTER_FLAG_HOST_REQUIRED)){
	...
	if(empty($errors)){
		...
		$query = $dbh->prepare('INSERT INTO movies(title, actors, director, producer, year_of_prod, language, category, storyline, video) VALUES(:title, :actors, :director, :producer, :year_of_prod, :language, :category, :storyline, :video)');
...
<body>
	...
	<?php if($_POST): ?>
	<?php if(empty($errors)): ?>
		<p class="success">Le film a bien été ajouté !</p>
	<?php endif; ?>
	<?php endif; ?>
